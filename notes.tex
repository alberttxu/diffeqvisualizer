...
\dot{v} = Av = \lambda v
\end{equation}
which is actually 1-D in each component,
i.e. $\dot{v}_\mathrm{horiz} = \lambda v_\mathrm{horiz}$, $\dot{v}_\mathrm{vert} = \lambda v_\mathrm{vert}$.
This means that $v$ travels either directly towards or away from the origin, but its angle never changes.
And just like in one dimension, $\lambda < 0$ leads to shrinking, and $\lambda > 0$ leads to growth.
...
\end{equation}
Here $x(t)$ and $x_\mathrm{initial}$ are vectors, and $e^{tA}$ is a $2\times 2$ matrix --- this is called the matrix exponential function applied to $tA$.
